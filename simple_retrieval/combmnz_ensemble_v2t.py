#!/usr/bin/env python
"""
CombMNZ (Combined Multiple Non-Zero) for Video-to-Text Retrieval (V2T)
No GPU required - pure numpy implementation

CombMNZ: Similar to CombSum but multiplies by the number of non-zero scores
Formula: score(text) = (sum of normalized scores) × (number of systems with non-zero scores)

This rewards texts that appear in multiple systems (consensus voting).
...
def combmnz_fusion(sim_matrices, normalization='minmax', weights=None, threshold=1e-6):
    """
    Perform CombMNZ fusion on multiple similarity matrices for V2T.
    ...
    Returns:
        fused_scores: numpy array of shape (num_videos, num_texts) with CombMNZ scores
    """
    ...
    nonzero_counts = np.zeros((num_videos, num_texts), dtype=np.int32)
    ...
    for v_idx in range(num_videos):
        ...
        for mat_idx, mat in enumerate(sim_matrices):
            ...
            nonzero_counts[v_idx] += nonzero_mask.astype(np.int32)
    ...
    print(f"  Texts with full consensus: {np.sum(nonzero_counts == len(sim_matrices))}")
...
def evaluate_v2t_retrieval(fused_scores, df, video_dir):
    ...
    for v_idx in range(num_videos):
        ...
        # Get CombMNZ scores for this video query
        video_scores = fused_scores[v_idx]
        
        # Rank texts by CombMNZ score (higher is better)
...
def main():
    parser = argparse.ArgumentParser(description="CombMNZ Fusion for Video-to-Text Retrieval (V2T)")
    ...
    parser.add_argument("--output_path", type=str, default="combmnz_fused_similarity_v2t.npy",
    ...
    print("=" * 80)
    print("CombMNZ (Combined Multiple Non-Zero) for Video-to-Text Retrieval (V2T)")
    print("=" * 80)
    print(f"\nConfiguration:")
