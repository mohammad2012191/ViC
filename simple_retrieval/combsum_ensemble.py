#!/usr/bin/env python
"""
CombSum (Score Fusion) for Text-to-Video Retrieval
No GPU required - pure numpy implementation

CombSum: Simply sums the normalized similarity scores from multiple systems
Formula: score(video) = sum over all systems of: normalized_score_in_system

Normalization options:
- min-max: (score - min) / (max - min) per query
- z-score: (score - mean) / std per query
- none: raw scores (only if scores are already comparable)
"""
...
def combsum_fusion(sim_matrices, normalization='minmax', weights=None):
    ...
    Returns:
        fused_scores: numpy array of shape (num_queries, num_videos) with CombSum scores
    """
...
def evaluate_retrieval(fused_scores, df, video_dir):
    ...
    for q_idx in range(num_queries):
        ...
        # Get CombSum scores for this query
        query_scores = fused_scores[q_idx]
        
        # Rank videos by CombSum score (higher is better)
...
def main():
    parser = argparse.ArgumentParser(description="CombSum Fusion for Text-to-Video Retrieval")
    ...
    parser.add_argument("--output_path", type=str, default="combsum_fused_similarity.npy",
    ...
    print("=" * 80)
    print("CombSum (Score Fusion) for Text-to-Video Retrieval")
    print("=" * 80)
    print(f"\nConfiguration:")
