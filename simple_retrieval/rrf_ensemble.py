#!/usr/bin/env python
"""
Reciprocal Rank Fusion (RRF) for Text-to-Video Retrieval
No GPU required - pure numpy implementation

RRF Formula: score(video) = sum over all systems of: 1 / (k + rank_in_system)
...
def reciprocal_rank_fusion(sim_matrices, k=60):
    ...
    Returns:
        fused_scores: numpy array of shape (num_queries, num_videos) with RRF scores
    """
    ...
    for q_idx in range(num_queries):
        ...
        for mat in sim_matrices:
            ...
            # Rank videos by similarity (higher is better)
            # argsort in descending order
            ranked_video_indices = np.argsort(-query_scores)
            
            # Assign RRF scores
            for rank_position, video_idx in enumerate(ranked_video_indices):
                # rank_position starts at 0, but rank should start at 1
                rank = rank_position + 1
                rrf_score = 1.0 / (k + rank)
                fused_scores[q_idx, video_idx] += rrf_score
...
def evaluate_retrieval(fused_scores, df, video_dir):
    ...
    for q_idx in range(num_queries):
        ...
        # Rank videos by RRF score (higher is better)
...
def main():
    parser = argparse.ArgumentParser(description="Reciprocal Rank Fusion for Text-to-Video Retrieval")
    ...
    parser.add_argument("--output_path", type=str, default="rrf_fused_similarity.npy",
    ...
    print("=" * 80)
    print("Reciprocal Rank Fusion (RRF) for Text-to-Video Retrieval")
    print("=" * 80)
    print(f"\nConfiguration:")
