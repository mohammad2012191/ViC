...
#####################################################
class Config:
    num_images = 14  # For t2v mode
    num_captions = 20  # For v2t mode
    ...
    retrieval_mode = "t2v"  # "t2v" or "v2t" - Set dynamically via command-line
...
class VLMWorker:
    ...
    def load_image_intern(self, image_pil, max_num=6):
        """
        Convert a PIL Image to pixel_values suitable for the InternVL2 model.
        """
        transform = self.transform
        images = self.dynamic_preprocess(image_pil, min_num=1, max_num=max_num, image_size=448, use_thumbnail=True)
    ...
    #####################################################
    # Reranking for Text-to-Video (t2v)
    #####################################################
    ...
    #####################################################
    # Reranking for Video-to-Text (v2t)
    #####################################################
    def rerank_top_captions(self, query_patches, text_candidates, video_id):
        """
        Given pixel_values for a single video query and a list of text candidates,
        build a prompt to rank those captions in relevance to the video.
        Return the new ranking.
        """
        num_captions = len(text_candidates)
        prompt_lines = []
    
        # The single video query
        prompt_lines.append("Image-1: <image>")
    
        # Add subtitle if enabled and available
        if Config.use_subs:
            sub = self.subs.get(video_id, "")
            if sub:
                prompt_lines.append(f"Subtitle-1: {sub}")
    
        # The text candidates
        for i, text_str in enumerate(text_candidates, start=1):
            prompt_lines.append(f"Candidate-{i}: {text_str}")
    
        # Create prompt based on whether subtitles are used
        if Config.use_subs:
            prompt_lines.append(
        ...
        # Feed the video patches and prompt to the model
        generation_config = dict(max_new_tokens=256, do_sample=False)
        try:
            response = self.model.chat(
                self.tokenizer,
                ...
                final_prompt,
                generation_config,
                num_patches_list=[query_patches.size(0)]
            )
        except Exception as e:
    ...
    def process_data_slice(self, data_slice, df_rows, sim_mats):
        """
        Process a slice of data based on retrieval mode (t2v or v2t).
        """
        if Config.retrieval_mode == "t2v":
            return self._process_t2v(data_slice, df_rows, sim_mats)
        elif Config.retrieval_mode == "v2t":
            return self._process_v2t(data_slice, df_rows, sim_mats)
        else:
            raise ValueError(f"Unknown retrieval_mode: {Config.retrieval_mode}")
    
    def _process_t2v(self, data_slice, df_rows, sim_mats):
        """
        Text-to-Video retrieval processing.
        ...
        for idx in tqdm(data_slice, desc=f"[GPU {self.gpu_id}] Processing T2V"):
            # user query
    ...
    def _process_v2t(self, data_slice, df_rows, sim_mats):
        """
        Video-to-Text retrieval processing.
        For each item in data_slice (these are row indices for *videos* now):
          ...
          3) We'll run rerank_top_captions(...) passing:
        ...
        for idx in tqdm(data_slice, desc=f"[GPU {self.gpu_id}] Processing V2T"):
            ...
            video_query_img = self.get_first_frame(video_path, grid_size=Config.grid_size)
