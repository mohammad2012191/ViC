...
import pandas as pd
import argparse
from ensemble_msrvtt_activitynet_didemo_t2v import run_distributed_inference
from ensemble_msrvtt_activitynet_didemo_t2v import Config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Distributed Text-to-Video Retrieval with InternVL")
    ...
    parser.add_argument("--num_images", type=int, required=True,
                        help="Number of video candidates to select from similarity matrix")
...
def main():
    ...
    # Note: NO transpose for t2v (texts are rows, videos are columns)
