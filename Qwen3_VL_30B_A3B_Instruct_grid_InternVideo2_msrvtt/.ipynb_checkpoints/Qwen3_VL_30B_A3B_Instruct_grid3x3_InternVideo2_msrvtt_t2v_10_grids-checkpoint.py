...
class VLMWorker:
    def __init__(self, gpu_id):
        """Initialize the Qwen3-VL model+processor on a specific GPU."""
        self.gpu_id = gpu_id
    ...
    def rerank_top_videos(
        ...
        fetch first frames, build patches, call model.generate to rerank.
        Return new ranking + recall metrics if we know the ground-truth ID.
        """
        # Load each video's first frame as PIL Image
        images_list = []
        for vid_path in video_paths:
            img = self.get_first_frame(vid_path)  # Returns PIL Image
            images_list.append(img)
    
        num_images = len(video_paths)
        
        # Build prompt for <num_images> images (same prompt as before)
        prompt_lines = []
        ...
        # Prepare messages in Qwen3-VL format with multiple images
        content = []
        ...
        # parse new ranking (same as before)
