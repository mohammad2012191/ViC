...
#####################################################
class Config:
    num_images = 10  # how many videos we select from similarity row 
    video_dir = "MSRVTT"
...
class VLMWorker:
    ...
    def rerank_top_videos(
        ...
        """
        Given a user query string and a list of video paths,
        fetch first frames and call model to rerank using Gemma 3.
        Return new ranking.
        """
        # Load each video's first frame (already 896x896)
        video_images = []
        for vid_path in video_paths:
            img = self.get_first_frame(vid_path)
        ...
        # Build content with images and labels
        content = []
        ...
        # Add the ranking instruction
        content.append({
            "type": "text",
            "text": (
                f"Given the user query: \"{query_text}\", rank these {num_images} images from most to least relevant "
                f"to the user query and output a list like [1,3,2,5,4,...,{num_images}] (just an example). "
                f"Output only the {num_images}-ranked-elements list instantly."
            )
        })
        ...
        messages = [
            {
                "role": "system",
                "content": [{"type": "text", "text": "You are a helpful assistant that ranks images based on relevance to text queries."}]
            },
            {
                "role": "user",
                "content": content
        ...
        try:
            ...
            print(f"[GPU {self.gpu_id}] Error during generation: {e}")
            # fallback
            response = "[" + ",".join(str(i+1) for i in range(num_images)) + "]"
        ...
        ranking = self.parse_ranking_from_response(response, num_images)
        return ranking
    def parse_ranking_from_response(self, response_text, num_images):
        """
        E.g. parse "[1,3,2,5,4]".
        Fallback if not found or invalid = [1..num_images].
        """
        ...
        if match:
            ...
            try:
                ...
                # Check if all values are in valid range [1, num_images]
                if not all(1 <= r <= num_images for r in ranks):
                    print(f"[GPU {self.gpu_id}] Invalid ranking values: {ranks}. Expected values in range [1, {num_images}]. Using original order.")
