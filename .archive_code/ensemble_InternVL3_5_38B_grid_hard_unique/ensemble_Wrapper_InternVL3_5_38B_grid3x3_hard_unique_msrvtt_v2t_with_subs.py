...
import numpy as np
import pandas as pd
from ensemble_InternVL3_5_38B_grid3x3_hard_unique_msrvtt_v2t_with_subs import run_distributed_inference
from ensemble_InternVL3_5_38B_grid3x3_hard_unique_msrvtt_v2t_with_subs import Config
...
def main():
    ...
    ##################################################
    csv_path = "MSRVTT_JSFUSION_test.csv"  # The CSV with columns: key, vid_key, video_id, sentence
    df = pd.read_csv(csv_path)
    df.sort_values('video_id', inplace=True)
    df.reset_index(drop=True, inplace=True)
        
    sim_paths = ["clip4clip_msrvtt.npy",
                 #"VAST_msrvtt.npy",
                 "GRAM_msrvtt_tvas.npy",
                 "InternVideo2_msrvtt.npy"] # pass list via env or CLI
