...
import numpy as np
import pandas as pd
from ensemble_InternVL3_5_38B_grid3x3_hard_unique_activitynet_v2t import run_distributed_inference
from ensemble_InternVL3_5_38B_grid3x3_hard_unique_activitynet_v2t import Config
...
def main():
    ...
    ##################################################
    csv_path = "descs_ret_test_activitynet.csv"  # The CSV with columns: key, vid_key, video_id, sentence
    df = pd.read_csv(csv_path)
    df.sort_values('video_id', inplace=True)
    df.reset_index(drop=True, inplace=True)
        
    sim_paths = ["clip4clip_activitynet.npy",
                 #"VAST_activitynet.npy",
                 "GRAM_activitynet_tva.npy",
                 "InternVideo2_activitynet.npy"] # pass list via env or CLI
    sim_mats  = [np.load(p).T for p in sim_paths]
