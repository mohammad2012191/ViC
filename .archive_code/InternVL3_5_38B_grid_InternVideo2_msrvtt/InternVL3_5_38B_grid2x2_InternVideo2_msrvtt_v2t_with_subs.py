...
class VLMWorker:
    ...
    def rerank_top_captions(self, query_patches, text_candidates, query_video_id):
        ...
        # Build the prompt:
        #   "Query-Video: <image>"
        #   "Query-Subtitle: <subtitle_text>"  (if available)
        #   "Candidate-1: <text>"
        #   "Candidate-2: <text>"
        #    ...
        #   "Given the user query (the video), rank these N text candidates..."
        prompt_lines = []
    ...
    def process_data_slice(self, data_slice, df_rows, sim_matrix):
        ...
        for idx in tqdm(data_slice, desc=f"[GPU {self.gpu_id}] Processing"):
            ...
            video_query_img = self.get_first_frame(video_path, grid_size=2)
